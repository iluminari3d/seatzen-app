...
function ArrangeOptionsModal({ onClose, onArrange, isArranging }) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 animate-fade-in">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-lg m-4">
						...
						<div className="space-y-2">
							{differentTables.map(table => (
								<div key={table.id} className="grid grid-cols-3 gap-2">
									...
									<input type="text" value={table.category} onChange={e => updateDifferentTable(table.id, 'category', e.target.value)} placeholder="Es. Testimoni" className="col-span-2 shadow appearance-none border rounded-lg py-2 px-3 text-gray-800 dark:text-gray-200 leading-tight focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 dark:border-gray-600 sz-focus-ring" />
                ...
                <div className="flex justify-end space-x-4 mt-8">
                    <button onClick={onClose} className="text-gray-600 dark:text-gray-300 font-bold py-2 px-4">Annulla</button>
                    <button onClick={handleArrange} disabled={isArranging} className="sz-accent-bg text-white font-bold py-2 px-6 rounded-lg shadow-lg sz-accent-bg-hover disabled:bg-gray-400">{isArranging ? 'Disponendo...' : 'Disponi'}</button>
                </div>
            </div>
        </div>
    );
...
function EventView({ event, db, user, onDeleteEvent }) {
    ...
    return (
        <>
			...
			{showArrangeOptions && <ArrangeOptionsModal onClose={() => setShowArrangeOptions(false)} onArrange={handleAutoArrange} isArranging={isArranging} />}
