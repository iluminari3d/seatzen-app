...
const DEFAULT_STRICT_GROUPS = ["Testimoni Sposo", "Testimoni Sposa", "Genitori Sposo", "Genitori Sposa", "Amici Sposo", "Amici Sposa", "Parenti Sposo", "Parenti Sposa"];
...
function ArrangeOptionsModal({ onClose, onArrange, isArranging, guestGroups }) {
	...
	const addDifferentTable = () => {
		setDifferentTables([...differentTables, { id: Date.now(), capacity: 8, categories: [] }]);
	}
	...
	const handleCategoryChange = (tableId, categoryId) => {
		const newDifferentTables = [...differentTables];
		const index = newDifferentTables.findIndex(table => table.id === tableId);
		const table = newDifferentTables[index];
		const categoryIndex = table.categories.indexOf(categoryId);
		if (categoryIndex > -1) {
			table.categories.splice(categoryIndex, 1);
		} else {
			table.categories.push(categoryId);
		}
		setDifferentTables(newDifferentTables);
	}
											...
											<div className="col-span-2">
												{guestGroups.map(group => (
													<div key={group.id} className="flex items-center">
														<input id={`${table.id}-${group.id}`} type="checkbox" checked={table.categories.includes(group.id)} onChange={() => handleCategoryChange(table.id, group.id)} className="h-4 w-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
														<label htmlFor={`${table.id}-${group.id}`} className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{group.name}</label>
													</div>
												))}
											</div>
							...
							{allowDifferentTables && differentTables.length > 0 && (
								<div>
									...
									<ul className="list-disc list-inside">
										{differentTables.map(table => (
											<li key={table.id}>Tavolo per "{table.categories.map(c => guestGroups.find(g => g.id === c)?.name).join(', ')}" da {table.capacity} posti</li>
										))}
									</ul>
								</div>
							)}
...
function EventView({ event, db, user, onDeleteEvent }) {
	...
	const handleSavePerson = async (personId, personData) => {
		if (!db || !user || !event) return;
		setIsSavingPerson(true);
		const peopleCollection = activeSection === 'guests' ? 'guests' : 'staff';
		const path = `artifacts/${appId}/users/${user.uid}/events/${event.id}/${peopleCollection}`;
		
		if (personId) {
			await updateDoc(doc(db, path, personId), personData);
		} else {
        ...
        <>
            ...
            <div className="w-full h-full p-4 md:p-8 text-left animate-fade-in dark:bg-gray-900" onDragLeave={() => {}}>
                ...
                <div className="flex justify-between items-start mb-8">
					...
					<div className="flex items-center space-x-2">
						<button onClick={() => alert("Funzionalità in arrivo!")} className="text-sm font-bold text-gray-600 dark:text-gray-300 hover:underline">Tutti i Partecipanti</button>
