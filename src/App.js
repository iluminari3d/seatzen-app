...
function PersonDetailModal({ person, allGuests, strictGroups, onSave, onClose, isSaving, isCreating, onManageGroups, onDelete, initialType = 'guest' }) {
    ...
    const handleDeleteClick = () => {
        ...
        setTimeout(() => onDelete(person), 150); // Then open the confirmation modal after a short delay
    }
        ...
        <div className="w-full p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-inner min-h-[300px] border border-gray-200 dark:border-gray-700">
            <div className="flex flex-wrap justify-center items-start gap-8 p-4">
                {tables.length > 0 ? tables.map(table => {
                    ...
                    const tableSize = table.capacity > 10 ? (isRound ? 'w-56 h-56' : 'w-72') : (isRound ? 'w-48 h-48' : 'w-64');
                    const tableClasses = `flex flex-col justify-center items-center p-4 border-2 border-gray-600 dark:border-gray-400 bg-white dark:bg-gray-700 shadow-lg ${isRound ? 'rounded-full' : 'rounded-lg'} ${tableSize}`;
                    return (
                        <div key={table.id} className={`${tableClasses} transition-transform hover:scale-105`}>
                            ...
                            <ul className="text-center text-sm text-gray-700 dark:text-gray-300">
                                {assignedPeople.map(p => <li key={p.id} className="truncate max-w-[150px]">{p.name}</li>)}
                            </ul>
                        </div>
                    );
...
function ArrangeOptionsModal({ onClose, onArrange, isArranging, guestGroups }) {
    ...
    const availableGroups = guestGroups.filter(g => g.name.toLowerCase() !== 'sposi');

    const handleAddSpecialTable = () => {
        setSpecialTables([...specialTables, { id: Date.now(), groupIds: [], capacity: 8 }]);
    };
    
    const handleRemoveSpecialTable = (id) => {
        setSpecialTables(specialTables.filter(t => t.id !== id));
    };

    const updateSpecialTable = (id, field, value) => {
        const newSpecialTables = specialTables.map(t => {
            if (t.id === id) {
                return { ...t, [field]: value };
            }
            return t;
        });
        setSpecialTables(newSpecialTables);
    };
    ...
    const handleArrange = () => {
        onArrange({
            ...
            createKidsTable,
            allowSpecialTables,
            specialTables: allowSpecialTables ? specialTables.filter(t => t.groupIds.length > 0) : []
        });
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 animate-fade-in">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-2xl m-4 max-h-[90vh] flex flex-col">
                <h2 style={{ fontFamily: 'Lora, serif' }} className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">Opzioni Auto-Disposizione</h2>
                
                <div className="flex-grow overflow-y-auto pr-4 -mr-4 space-y-6">
                    {/* Standard Table Options */}
                    <div>
                        <h3 className="font-bold text-lg mb-3 text-gray-700 dark:text-gray-300 border-b pb-2">Impostazioni Standard</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="font-bold text-gray-700 dark:text-gray-300 mb-2 block text-sm">Capacità Tavoli</label>
                                <input type="number" min="1" value={capacity} onChange={(e) => setCapacity(parseInt(e.target.value, 10))} className="shadow-sm appearance-none border rounded-lg w-full py-2 px-3 text-gray-800 dark:text-gray-200 leading-tight focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 dark:border-gray-600 sz-focus-ring" />
                            </div>
                            <div>
                                <label className="font-bold text-gray-700 dark:text-gray-300 mb-2 block text-sm">Forma Tavoli</label>
                                <select value={tableType} onChange={(e) => setTableType(e.target.value)} className="shadow-sm border rounded-lg w-full py-2 px-3 text-gray-800 dark:text-gray-200 leading-tight focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 dark:border-gray-600 sz-focus-ring">
                                    <option value="round">Rotondo</option>
                                    <option value="rectangular">Imperiale</option>
                                </select>
                            </div>
                    ...
                    <div>
                        <h3 className="font-bold text-lg mb-3 text-gray-700 dark:text-gray-300 border-b pb-2">Gruppi Specifici</h3>
                        <div className="space-y-3">
                            <div className="flex items-center">
                                <input type="checkbox" id="noCoupleTable" checked={noCoupleTable} onChange={(e) => setNoCoupleTable(e.target.checked)} className="h-4 w-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                                <label htmlFor="noCoupleTable" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Non creare un tavolo separato per gli Sposi</label>
                            </div>
                             <div className="flex items-center">
                                <input type="checkbox" id="createKidsTable" checked={createKidsTable} onChange={(e) => setCreateKidsTable(e.target.checked)} className="h-4 w-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                                <label htmlFor="createKidsTable" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Crea Tavolo Bambini separato</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex justify-between items-center border-b pb-2">
                             <h3 className="font-bold text-lg text-gray-700 dark:text-gray-300">Tavoli Speciali</h3>
                             <label htmlFor="allowSpecialTablesToggle" className="flex items-center cursor-pointer">
                                <div className="relative">
                                    <input type="checkbox" id="allowSpecialTablesToggle" className="sr-only" checked={allowSpecialTables} onChange={() => setAllowSpecialTables(!allowSpecialTables)} />
                                    <div className={`block w-12 h-7 rounded-full transition-colors ${allowSpecialTables ? 'sz-accent-bg' : 'bg-gray-300 dark:bg-gray-600'}`}></div>
                                    <div className={`dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition-transform ${allowSpecialTables ? 'translate-x-full' : ''}`}></div>
                                </div>
                            </label>
                        </div>
                        ...
                        {allowSpecialTables && (
                            <div className="mt-4 space-y-4 animate-fade-in">
                                {specialTables.map((table, index) => (
                                    <div key={table.id} className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg border dark:border-gray-600">
                                        <div className="flex justify-between items-center mb-2">
                                            <p className="text-sm font-bold text-gray-600 dark:text-gray-300">Regola Speciale {index + 1}</p>
                                            <button onClick={() => handleRemoveSpecialTable(table.id)} className="text-red-500 hover:text-red-700">
                                                <TrashIcon className="h-4 w-4"/>
                                            </button>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div>
                                                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 block mb-1">Gruppi</label>
                                                <select
                                                    multiple
                                                    value={table.groupIds}
                                                    onChange={e => updateSpecialTable(table.id, 'groupIds', Array.from(e.target.selectedOptions, option => option.value))}
                                                    className="shadow-sm border rounded-lg w-full p-2 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 dark:border-gray-600 sz-focus-ring"
                                                >
                                                    {availableGroups.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                                                </select>
                                            </div>
                                             <div>
                                                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 block mb-1">Capacità Tavolo</label>
                                                <input
                                                    type="number"
                                                    min="1"
                                                    value={table.capacity}
                                                    onChange={e => updateSpecialTable(table.id, 'capacity', parseInt(e.target.value, 10))}
                                                    className="shadow-sm appearance-none border rounded-lg w-full py-2 px-3 text-sm text-gray-800 dark:text-gray-200 leading-tight focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 dark:border-gray-600 sz-focus-ring"
                                                />
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                <button onClick={handleAddSpecialTable} className="text-sm sz-accent-text font-bold hover:underline self-start">+ Aggiungi regola</button>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flex justify-end space-x-4 mt-8 pt-6 border-t dark:border-gray-700">
                    <button onClick={onClose} className="text-gray-600 dark:text-gray-300 font-bold py-2 px-4">Annulla</button>
                    <button onClick={handleArrange} disabled={isArranging} className="sz-accent-bg text-white font-bold py-2 px-6 rounded-lg shadow-lg sz-accent-bg-hover disabled:bg-gray-400">
                        {isArranging ? 'Creazione...' : 'Disponi'}
                    </button>
                </div>
            </div>
        </div>
...
function EventView({ event, db, user, onDeleteEvent }) {
    ...
    const handleExportPdf = (type) => {
        ...
        if (type === 'guests') {
            ...
            doc.autoTable({ startY: 25, head, body });
        ...
        if (type === 'tables') {
            ...
            doc.autoTable({ startY: 25, head, body, styles: { cellPadding: 2 }, headStyles: { fillColor: [41, 128, 185] } });
